...
Yet there is by now mounting evidence of widespread misuse so eggregious that it completely undermines the desired guarantees.  The overarching aim of this project is to reduce or even eliminate such errors. 

Our first research hypothesis is that many of the errors are due to a mismatch between developers' perception and actual guarantees that APIs offered, and that this mismatch can be qualitatively, or even quantitatively established through mental models of how developers use security API.
Our second research hypotesis (which naturally follows from the previous one) is that APIs can be improved through better abstractions which better reflect developer expectations of the underlying security functionality. 

These research questions translate to the following two objectives:
\begin{itemize}
\item[O1:] to build mental models for the use of security APIs by developers. 

\item[O2:] to develop improved APIs which overlap better to developer's expectations.
\end{itemize}
