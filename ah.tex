...
%For example, how to ensure authentication of parties, how to protect against replay attacks. 
...
%This kind of errors are often encountered in designs, and reflect 
...
This studenship proposes to further study the 
...
We structure the project along two direction. 
...
In this work package we will capitalise on the models built in WP 1 and design new abstractions which align better with 

. to eliminate the need for cryptographic expertise when translating applications designed within \spec\ to an implementation. 
Specifically, we will support developers to use cryptographic libraries via interfaces that closely match the abstractions used in the \spec.
For example, using the tools developed in this task, an implementer will be able to establish a secure channel between two parties by invoking a high-level command that obscures the underlying cryptographic mechanisms.  
In particular, employing TLS will not require the implementer to set up parameters or ensure certificate validation.  
Technically, we will provide wrappers to some of the most commonly used libraries.  The interface exposed by these wrappers will follow closely the level of abstraction of the language \spec. 
We remark that this approach is one of the feature of the NaCl cryptographic library~\cite{bernstein}; currently that library only considers lower level primitives.  
